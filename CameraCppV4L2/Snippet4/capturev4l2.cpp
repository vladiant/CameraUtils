...
int init_mmap(int fd)
{
    ...
    gBuffer = static_cast<uint8_t*>(mmap (nullptr, buf.length, PROT_READ | PROT_WRITE, MAP_SHARED, fd, buf.m.offset));
    printf("Length: %d\nAddress: %p\n", buf.length, gBuffer);
...
if (V4L2_PIX_FMT_MJPEG == gFmt.fmt.pix.pixelformat) {
    ...
    CvMat cvmat = cvMat(gFmt.fmt.pix.height, gFmt.fmt.pix.width, CV_8UC3, (void*)gBuffer);
    ...
    CvMat cvmat1 = cvMat(gFmt.fmt.pix.height, gFmt.fmt.pix.width, CV_8UC2, (void*)gBuffer);
    CvMat* cvmat = cvCreateMat(gFmt.fmt.pix.height, gFmt.fmt.pix.width, CV_8UC3);
